...
def detect_pitch(path: str) -> int | None:
    """Detect the pitch of a sample using the enhanced detection system.
    
    Uses the same intelligent detection logic as the main XPM converter
    with sample-type awareness and improved accuracy.
    ...
    # Import the enhanced detect_sample_note function from the main module
    try:
        # Try to import the enhanced detection function directly
        import importlib.util
        import sys
        
        # Get the path to the main module
        script_dir = os.path.dirname(os.path.abspath(__file__))
        main_module_path = os.path.join(script_dir, 'Gemini wav_TO_XpmV2.py')
        
        # Load the module
        spec = importlib.util.spec_from_file_location("main_converter", main_module_path)
        if spec and spec.loader:
            main_module = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(main_module)
            
            # Use the enhanced detection function
            if hasattr(main_module, 'detect_sample_note'):
                midi = main_module.detect_sample_note(path)
                if midi is not None:
                    logging.debug(f"Enhanced detection: {midi} for {os.path.basename(path)}")
                    return midi
                    
    except Exception as e:
        logging.debug(f"Could not use enhanced detection, falling back to legacy: {e}")
    
    # Fallback to the legacy detection methods if enhanced fails
    return detect_pitch_legacy(path)


def detect_pitch_legacy(path: str) -> int | None:
    """Legacy pitch detection method (fallback).
    
    Returns:
        int or None: MIDI note number if detected, None if no pitch could be determined
    """
    logging.debug(f"Using legacy detection for: {os.path.basename(path)}")
    ...
    logging.debug(f"No pitch detected for {path}")
    return None
